...
export async function GET(){const gate=await requireOperatorApi();if(!gate.ok)return gate.response;const s=getSupabaseAdmin();if(!s)return NextResponse.json({ok:false,error:"Supabase není nakonfigurovaný."},{status:503});let doc:any=null;try{const{data:source,error}=await s.storage.from(BUCKET).download(SOURCE_PATH);if(error||!source)throw new Error(error?.message||"BILLA PDF chybí.");const bytes=new Uint8Array(await source.arrayBuffer());doc=await loadPdfDocument(bytes);const words=await pageWords(doc,1);const candidates=extractLeafletCandidates(words,{pageNo:1,validFrom:"2026-08-19",validTo:"2026-08-25"});
  ...
  const failures=checks.filter(c=>!c.ok).map(c=>c.id);const standardFailures=standardChecks.filter(c=>!c.ok).map(c=>c.id);const loyaltyFailures=loyaltyChecks.filter(c=>!c.ok).map(c=>c.id);const blockFailures=blockChecks.filter(c=>!c.ok).map(c=>c.id);
  const regressionChecks=[
    {id:"block_count_15",ok:candidates.length===15},
    {id:"known_neighbor_contamination_zero",ok:knownContamination.length===0},
    {id:"kure_not_slanina",ok:atPrice(49.9).every(r=>!/Anglická|slanina/i.test(String(r.source_text||"")))},
    {id:"toaletni_not_pilsner",ok:atPrice(99.9).every(r=>!/(?:Pilsner|Urquell)/i.test(String(r.source_text||"")))},
    {id:"pilsner_not_maslo",ok:atPrice(28.83).every(r=>!/Jihočeské\s+máslo/i.test(String(r.source_text||"")))},
    {id:"maslo_not_pilsner",ok:atPrice(32.9).every(r=>!/(?:Pilsner|Urquell)/i.test(String(r.source_text||"")))},
    {id:"brambory_hrozny_separate",ok:atPrice(9.9).some(r=>/Brambory/i.test(String(r.source_text||""))&&!/Hrozny/i.test(String(r.source_text||"")))&&atPrice(39.9).some(r=>/Hrozny/i.test(String(r.source_text||""))&&!/Brambory/i.test(String(r.source_text||"")))},
    {id:"rajcata_neighbor_separate",ok:atPrice(39.9).some(r=>/Rajčata/i.test(String(r.source_text||""))&&!/(?:Brambory|Hrozny)/i.test(String(r.source_text||"")))},
  ];
  const regressionFailures=regressionChecks.filter(c=>!c.ok).map(c=>c.id);const point4gPass=failures.length===0&&standardFailures.length===0&&loyaltyFailures.length===0&&blockFailures.length===0&&regressionFailures.length===0;
  const named=candidates.filter(r=>typeof r.product_name==="string"&&r.product_name.trim());const quarantine=candidates.filter(r=>r.status==="quarantine");const debugWords=words.filter(w=>DEBUG_RE.test(w.text.trim())).map(w=>({text:w.text,x:w.x,y:w.y,w:w.w,h:w.h}));
  return NextResponse.json({ok:true,page_no:1,extractor_version:candidates[0]?.extractor_version??null,total:candidates.length,named:named.length,quarantine:quarantine.length,suspiciously_mixed:genericMixed.length,known_contamination:knownContamination.length,point_4c:{pass:failures.length===0,checks,failures},point_4d:{pass:standardFailures.length===0,checks:standardChecks,failures:standardFailures},point_4e:{pass:loyaltyFailures.length===0,checks:loyaltyChecks,failures:loyaltyFailures},point_4f:{pass:blockFailures.length===0,checks:blockChecks,failures:blockFailures},point_4g:{pass:point4gPass,checks:regressionChecks,failures:regressionFailures},debug_words:debugWords,debug_regions:{kuře_49_90:region(words,275,440,575,710),pilsner_99_90:region(words,275,445,95,230),price_28_83:region(words,275,445,0,130)},candidates:candidates.map(r=>({key:r.candidate_key,name:r.product_name,price:r.price_sale,standard:r.price_standard,loyalty:r.price_loyalty,without_loyalty:r.price_without_loyalty,loyalty_required:r.loyalty_required,pack:r.pack_text,pack_qty:r.pack_qty,pack_unit:r.pack_unit,pack_unit_qty:r.pack_unit_qty,unit_price:r.price_per_unit,unit_price_unit:r.price_per_unit_unit,status:r.status,reason:r.review_reason,source:r.source_text,bounds:r.extraction_payload?.block_bounds??null,name_candidates:r.extraction_payload?.name_candidates??[]}))});
