...
function region(words: OcrWord[], x0: number, x1: number, y0: number, y1: number) {
  return words.filter((w)=>w.x+w.w/2>=x0&&w.x+w.w/2<=x1&&w.y+w.h/2>=y0&&w.y+w.h/2<=y1)
...
export async function GET() {
  ...
  try {
    ...
    const bytes=new Uint8Array(await source.arrayBuffer());doc=await loadPdfDocument(bytes);const words=await pageWords(doc,1);
