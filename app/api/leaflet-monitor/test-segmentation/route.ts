...
export async function GET(){const gate=await requireOperatorApi();if(!gate.ok)return gate.response;const s=getSupabaseAdmin();if(!s)return NextResponse.json({ok:false,error:"Supabase není nakonfigurovaný."},{status:503});let doc:any=null;try{const{data:source,error}=await s.storage.from(BUCKET).download(SOURCE_PATH);if(error||!source)throw new Error(error?.message||"BILLA PDF chybí.");const bytes=new Uint8Array(await source.arrayBuffer());doc=await loadPdfDocument(bytes);const words=await pageWords(doc,1);const candidates=extractLeafletCandidates(words,{pageNo:1,validFrom:"2026-08-19",validTo:"2026-08-25"});
  ...
  const standardChecks=[
    {id:"kure_std_89_90",ok:atPrice(49.9).some(r=>Math.abs(Number(r.price_standard)-89.9)<0.001)},
    {id:"toaletni_std_139_90",ok:atPrice(99.9).some(r=>Math.abs(Number(r.price_standard)-139.9)<0.001)},
    {id:"maslo_std_44_90",ok:atPrice(32.9).some(r=>Math.abs(Number(r.price_standard)-44.9)<0.001)},
    {id:"vejce_std_139_90",ok:atPrice(84.9).some(r=>Math.abs(Number(r.price_standard)-139.9)<0.001)},
    {id:"veprova_std_209_90",ok:atPrice(74.9).some(r=>Math.abs(Number(r.price_standard)-209.9)<0.001)},
    {id:"toffifee_std_54_90",ok:atPrice(29.9).some(r=>Math.abs(Number(r.price_standard)-54.9)<0.001)},
    {id:"tvaroh_std_29_90",ok:atPrice(18.9).some(r=>Math.abs(Number(r.price_standard)-29.9)<0.001)},
    {id:"brambory_std_24_90",ok:atPrice(9.9).some(r=>Math.abs(Number(r.price_standard)-24.9)<0.001)},
  ];
  const loyaltyChecks=[
    {id:"maslo_loyalty_32_90",ok:atPrice(32.9).some(r=>Math.abs(Number(r.price_loyalty)-32.9)<0.001&&Math.abs(Number(r.price_without_loyalty)-44.9)<0.001&&r.loyalty_required===true)},
    {id:"vejce_loyalty_84_90",ok:atPrice(84.9).some(r=>Math.abs(Number(r.price_loyalty)-84.9)<0.001&&Math.abs(Number(r.price_without_loyalty)-139.9)<0.001&&r.loyalty_required===true)},
    {id:"toffifee_loyalty_29_90",ok:atPrice(29.9).some(r=>Math.abs(Number(r.price_loyalty)-29.9)<0.001&&Math.abs(Number(r.price_without_loyalty)-54.9)<0.001&&r.loyalty_required===true)},
    {id:"kure_not_loyalty",ok:atPrice(49.9).some(r=>r.price_loyalty==null&&r.price_without_loyalty==null&&r.loyalty_required==null)},
  ];
  const failures=checks.filter(c=>!c.ok).map(c=>c.id);const standardFailures=standardChecks.filter(c=>!c.ok).map(c=>c.id);const loyaltyFailures=loyaltyChecks.filter(c=>!c.ok).map(c=>c.id);const named=candidates.filter(r=>typeof r.product_name==="string"&&r.product_name.trim());const quarantine=candidates.filter(r=>r.status==="quarantine");const debugWords=words.filter(w=>DEBUG_RE.test(w.text.trim())).map(w=>({text:w.text,x:w.x,y:w.y,w:w.w,h:w.h}));
  return NextResponse.json({ok:true,page_no:1,extractor_version:candidates[0]?.extractor_version??null,total:candidates.length,named:named.length,quarantine:quarantine.length,suspiciously_mixed:genericMixed.length,known_contamination:knownContamination.length,point_4c:{pass:failures.length===0,checks,failures},point_4d:{pass:standardFailures.length===0,checks:standardChecks,failures:standardFailures},point_4e:{pass:loyaltyFailures.length===0,checks:loyaltyChecks,failures:loyaltyFailures},debug_words:debugWords,debug_regions:{kuře_49_90:region(words,275,440,575,710),pilsner_99_90:region(words,275,445,95,230),price_28_83:region(words,275,445,0,130)},candidates:candidates.map(r=>({key:r.candidate_key,name:r.product_name,price:r.price_sale,standard:r.price_standard,loyalty:r.price_loyalty,without_loyalty:r.price_without_loyalty,loyalty_required:r.loyalty_required,pack:r.pack_text,status:r.status,reason:r.review_reason,source:r.source_text,bounds:r.extraction_payload?.block_bounds??null,name_candidates:r.extraction_payload?.name_candidates??[]}))});
