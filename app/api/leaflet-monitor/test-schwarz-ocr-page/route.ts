...
import {extractWordsFromImageBuffer,extractWordsFromLeafletImageBuffer} from "@/lib/ocr";
...
export async function GET(req:NextRequest){
  ...
  const height=Math.max(600,Math.min(2400,Number(req.nextUrl.searchParams.get("height")||2400)));
  const mode=req.nextUrl.searchParams.get("mode")||"tiled";
  ...
  const words=mode==="tiled"?await extractWordsFromLeafletImageBuffer(buf):await extractWordsFromImageBuffer(buf);
  ...
  return NextResponse.json({ok:true,page:p,height,mode,image_url:imageUrl,original_bytes:original.length,image_bytes:buf.length,ocr_ms:Date.now()-started,word_count:words.length,words:words.slice(0,160),candidate_count:candidates.length,candidates:candidates.slice(0,30).map(c=>({name:c.product_name,price:c.price_sale,pack:c.pack_text,status:c.status,confidence:c.confidence,source:c.source_text}))});
}
