...
function inferCategory(name: string | null, brand: string | null): CategoryGuess | null {
  ...
  if (matches(text, /\b(mleko|polotucne|plnotucne)\b/)) {
    return { category: "Mléčné výrobky", subcategory: "Mléko" };
  }
  if (matches(text, /\b(maslo)\b/)) {
    return { category: "Mléčné výrobky", subcategory: "Máslo" };
  }
  if (matches(text, /\b(jogurt|jogurty)\b/)) {
    return { category: "Mléčné výrobky", subcategory: "Jogurty" };
  }
  if (matches(text, /\b(syr|eidam|gouda|niva)\b/)) {
    return { category: "Mléčné výrobky", subcategory: "Sýry" };
  }
  if (matches(text, /\b(sul|horcice|kecup|ketchup)\b/)) {
    return { category: "Potraviny", subcategory: "Dochucovadla" };
  }
  if (matches(text, /\b(testoviny|penne|spagety|ryze)\b/)) {
    return { category: "Potraviny", subcategory: "Těstoviny a rýže" };
  }
  if (matches(text, /\b(mouka|cukr|krupice|strouhanka)\b/)) {
    return { category: "Potraviny", subcategory: "Mouka a pečení" };
  }
  if (matches(text, /\b(fazole|kukurice|konzerv)\b/)) {
    return { category: "Potraviny", subcategory: "Konzervy" };
  }
  if (matches(text, /\b(tunak|losos|sardinky)\b/)) {
    return { category: "Ryby", subcategory: "Rybí konzervy" };
  }
  if (matches(text, /\b(pastika)\b/)) {
    return { category: "Masné výrobky", subcategory: "Paštiky" };
  }
  if (matches(text, /\b(caj)\b/)) {
    return { category: "Káva a čaj", subcategory: "Čaj" };
  }
  if (matches(text, /\b(kava)\b/)) {
    return { category: "Káva a čaj", subcategory: "Káva" };
  }
  if (matches(text, /\b(olej)\b/)) {
    return { category: "Potraviny", subcategory: "Oleje" };
  }
  if (matches(text, /\b(ovesne vlocky|vlocky|cerealie|cornflakes|lupinky)\b/)) {
    return { category: "Snídaně", subcategory: "Cereálie" };
  }
  if (matches(text, /\b(banan|banany|jablko|jablka|citron|citrusy)\b/)) {
    return { category: "Ovoce a zelenina", subcategory: "Ovoce" };
  }
  if (matches(text, /\b(rajcata|rajce|brambory|mrkev|cibule|brokolice|salat)\b/)) {
    return { category: "Ovoce a zelenina", subcategory: "Zelenina" };
  }
  if (matches(text, /\b(chleb|rohlik|housky|pecivo)\b/)) {
    return { category: "Pečivo", subcategory: "Pečivo" };
  }
...
function isBadMilkFallback(category: string | null, subcategory: string | null, name: string | null) {
  ...
  const looksLikeMilkBucket =
    (cat === "mlecne vyrobky" || cat === "mlecne") &&
    (sub === "trvanlive mleko" || sub === "mleko");
...
function mapOfferRow(row: AnyRow, sourceTable: string): SetrikPublicOffer {
  ...
  const rawCategory = first(
    str(row.category_canonical),
    str(row.category),
    str(row.category_name)
  );
  const rawSubcategory = first(
    str(row.subcategory_canonical),
    str(row.subcategory),
  ...
  return {
    ...
    unit: first(
      str(row.normalized_price_unit),
      str(row.unit_price_unit),
      str(row.pack_unit),
      str(row.unit),
