...
export function resolvePromoEvidence(candidate:ExtractedCandidate,words:OcrWord[]):PromoMatch|null{
 ...
 // A threshold printed as "OD N KS" belongs to the lower multi-buy price and wins over a nearby 1-piece reference price.
 ...
 // Conservative fallback: accept only a complete explicit phrase in one OCR item.
...
export function applyPromoEvidence(candidate:ExtractedCandidate,words:OcrWord[]):ExtractedCandidate{
 const found=resolvePromoEvidence(candidate,words);if(!found)return candidate;
 return{...candidate,minimum_quantity:found.minimum_quantity,promo_condition:found.raw,field_evidence:{...candidate.field_evidence,promo_condition:{raw_text:found.raw,source:found.source}},extraction_payload:{...candidate.extraction_payload,promo_resolution:found}};
}
