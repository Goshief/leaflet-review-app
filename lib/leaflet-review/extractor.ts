...
export const EXTRACTOR_VERSION = "leaflet-layout-v9";
...
function lineObjects(words: OcrWord[], medianH: number): Line[] {
  const rows=clusterIntoLines(words,Math.max(4,medianH*0.52));const out:Line[]=[];const gapLimit=Math.max(9,medianH*1.2);
