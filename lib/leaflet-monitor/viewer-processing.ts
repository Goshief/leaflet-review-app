...
async function ensureImport(s: any, retailer: RetailerId, path: string, sourceUrl: string) {
  ...
  const { data, error } = await s.from("imports").insert({
    ...
    import_contract_version: "leaflet-pages-v1",
...
async function findOrCreateDocument(s: any, retailer: RetailerId, manifest: Awaited<ReturnType<typeof resolveViewerPageManifest>>, manifestPath: string, importId: string) {
  const { data: candidates, error: findError } = await s.from("leaflet_documents")
    .select("*")
...
export async function ingestViewerPages(s: any, retailer: RetailerId, viewerUrl: string) {
  ...
  const { data: existing, error: pageLookupError } = await s.from("leaflet_page_processing")
  ...
  const missing = manifest.pages.filter((p) => !byPage.has(p.page_no)).map((p) => ({
    leaflet_id: doc.id,
    page_no: p.page_no,
    status: "pending",
    attempt_count: 0,
  ...
  const finalStatus = failed > 0 ? "failed" : completed === manifest.page_count ? ((Number(doc.candidate_count || 0) > 0) ? (doc.processing_status === "partially_reviewed" ? "partially_reviewed" : "ready_for_review") : "pages_ready") : "processing";
  const { data: finalDoc, error: finalError } = await s.from("leaflet_documents").update({
    page_count: manifest.page_count,
    processed_pages: completed,
    processing_status: finalStatus,
    processing_error: failed ? doc.processing_error : null,
    processing_completed_at: completed === manifest.page_count ? new Date().toISOString() : null,
    updated_at: new Date().toISOString(),
