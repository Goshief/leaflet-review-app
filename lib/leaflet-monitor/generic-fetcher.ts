...
function absoluteUrl(raw: string, baseUrl: string): string | null {
  try {
    return new URL(decodeHtml(raw.trim()).replace(/\\u0026/gi, "&"), baseUrl).toString();
  } catch {
    return null;
  }
}
...
function extractPdfUrls(html: string, baseUrl: string): string[] {
  ...
  const add = (raw: string) => {
    const value = absoluteUrl(raw, baseUrl);
    if (value && /\.pdf(?:$|[?#])/i.test(value)) found.add(value);
  };
  ...
  for (const m of html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    ...
    if (/pdf|download|stáhnout|stahnout/i.test(label)) add(m[1] ?? "");
  }
  return [...found];
}
...
function extractViewerCandidates(html: string, baseUrl: string, preferredLabels: RegExp[]): string[] {
  ...
  for (const m of html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    ...
    preferredLabels.forEach((rx, i) => { if (rx.test(haystack)) score += 20 - i; });
...
async function asPdf(response: Response): Promise<{ url: string; bytes: Uint8Array } | null> {
  if (!response.ok) return null;
  const bytes = new Uint8Array(await response.arrayBuffer());
  const type = response.headers.get("content-type") ?? "";
  const sig = new TextDecoder().decode(bytes.slice(0, 5));
  if (type.includes("application/pdf") || sig === "%PDF-") return { url: response.url, bytes };
  return null;
}

async function resolvePdf(sourceHtml: string, sourceUrl: string, preferredLabels: RegExp[]): Promise<{ url: string; bytes: Uint8Array; viewer_url: string | null }> {
  const direct = extractPdfUrls(sourceHtml, sourceUrl);
  for (const url of direct.slice(0, 12)) {
    try {
      const hit = await asPdf(await fetchWithTimeout(url));
      if (hit) return { ...hit, viewer_url: sourceUrl };
    } catch {}
  }

  const viewers = extractViewerCandidates(sourceHtml, sourceUrl, preferredLabels);
  for (const viewerUrl of viewers) {
    try {
      const viewer = await fetchWithTimeout(viewerUrl);
      ...
      for (const pdfUrl of extractPdfUrls(html, viewer.url || viewerUrl).slice(0, 12)) {
        try {
          const hit = await asPdf(await fetchWithTimeout(pdfUrl));
          if (hit) return { ...hit, viewer_url: viewerUrl };
        } catch {}
      }
    } catch {}
  }
...
async function readLearning(supabase: any, retailer: RetailerId): Promise<RetailerLearningState | null> {
  const { data, error } = await supabase.storage.from(BUCKET).download(`_learning/${retailer}.json`);
  if (error || !data) return null;
  try { return JSON.parse(await data.text()) as RetailerLearningState; } catch { return null; }
}
...
export async function runGenericLeafletConnector(req: Request, config: Config) {
  ...
  } else if (schedule !== config.cronSchedule) {
  ...
  if (!decision.due) {
    return NextResponse.json({ ok: true, status: "adaptive_skip", retailer: config.retailer, reason: decision.reason, checks_this_week: decision.checks_this_week, next_check_at: learning?.next_check_at ?? null });
  }
  ...
  if ((existingMarker ?? []).some((x: any) => x.name === `${config.retailer}-${todayPrague()}.json`)) {
    return NextResponse.json({ ok: true, status: "already_checked_today", retailer: config.retailer });
  }
  ...
  try {
    ...
    const pdf = await resolvePdf(sourceHtml, source.url || config.sourcePage, config.preferredLabels);
    const sha256 = createHash("sha256").update(pdf.bytes).digest("hex");
    const shortSha = sha256.slice(0, 16);
    const { data: existing } = await supabase.storage.from(BUCKET).list(config.retailer, { search: shortSha, limit: 20 });
    const alreadyStored = (existing ?? []).some((x: any) => x.name?.includes(shortSha));

    if (alreadyStored) {
      const payload = { checked_at: checkedAt, visited_url: config.sourcePage, retailer: config.retailer, status: "unchanged", viewer_url: pdf.viewer_url, pdf_url: pdf.url, sha256 };
      await supabase.storage.from(BUCKET).upload(marker, new Blob([JSON.stringify(payload)], { type: "application/json" }), { contentType: "application/json", upsert: true });
      const next = recordObservation(learning, config.retailer, "unchanged", checkedAt, config.sourcePage);
      await writeLearning(supabase, next);
    ...
    const { error: uploadError } = await supabase.storage.from(BUCKET).upload(storagePath, pdf.bytes, { contentType: "application/pdf", upsert: false, cacheControl: "3600" });
    if (uploadError) throw new Error(`Storage upload: ${uploadError.message}`);

    const payload = { checked_at: checkedAt, visited_url: config.sourcePage, retailer: config.retailer, status: "downloaded", viewer_url: pdf.viewer_url, pdf_url: pdf.url, sha256, bytes: pdf.bytes.byteLength, storage_path: storagePath };
    await supabase.storage.from(BUCKET).upload(marker, new Blob([JSON.stringify(payload)], { type: "application/json" }), { contentType: "application/json", upsert: true });
    const next = recordObservation(learning, config.retailer, "downloaded", checkedAt, config.sourcePage);
    await writeLearning(supabase, next);
    return NextResponse.json({ ok: true, ...payload, learning: next });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    const payload = { checked_at: checkedAt, visited_url: config.sourcePage, retailer: config.retailer, status: "error", error: message };
    await supabase.storage.from(BUCKET).upload(marker, new Blob([JSON.stringify(payload)], { type: "application/json" }), { contentType: "application/json", upsert: true });
