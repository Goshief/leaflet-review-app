...
export function LeafletMonitorPanel() {
  ...
  async function testPage(row: RetailerRow) {
    if (!row.latest_pdf) return;
    const page = row.ai?.next_page && row.ai.next_page <= (row.ai.page_count ?? Infinity) ? row.ai.next_page : 1;
    setWorking(row.id);
    setError("");
    setReview(null);
    try {
      const response = await fetch("/api/leaflet-ai/process", {
        ...
        body: JSON.stringify({ bucket: "leaflet-intake", path: `${row.id}/${row.latest_pdf}`, retailer: row.id, page, dry_run: true }),
      });
      const data = await response.json();
      if (!response.ok || !data?.ok) throw new Error(data?.error ?? `HTTP ${response.status}`);
      setReview({
      ...
      setError(cause instanceof Error ? cause.message : "AI test stránky selhal.");
    } finally {
      setWorking(null);
    }
  }
  ...
  async function approveReview() {
    if (!review) return;
    const row = rows.find((x) => x.id === review.retailer);
    if (!row?.latest_pdf) return;
    setWorking(review.retailer);
    setError("");
    try {
      const response = await fetch("/api/leaflet-ai/approve", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ bucket: "leaflet-intake", path: `${row.id}/${row.latest_pdf}`, page: review.page, verified_offers: review.verified_offers }),
      });
      const data = await response.json();
      if (!response.ok || !data?.ok) throw new Error(data?.error ?? `HTTP ${response.status}`);
      setReview(null);
      ...
      setError(cause instanceof Error ? cause.message : "Schválení stránky selhalo.");
    } finally {
      setWorking(null);
    }
  }
    ...
    <section className="mb-10 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-slate-600">Systém eviduje skutečné návštěvy webů, učí se dny zveřejnění a na každý obchod pustí robota maximálně 2× týdně.</p>
        </div>
        ...
        {rows.map((row) => (
          <article key={row.id} className="rounded-2xl border border-slate-200 p-4">
            ...
            <dl className="mt-3 space-y-2 text-xs text-slate-600">
              ...
              {row.latest_pdf ? <div><dt className="font-semibold text-slate-500">Poslední PDF</dt><dd className="break-all">{row.latest_pdf}</dd></div> : null}
            </dl>
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {row.latest_pdf ? <button type="button" disabled={working === row.id} onClick={() => void testPage(row)} className="rounded-lg bg-slate-900 px-2.5 py-1.5 text-xs font-bold text-white disabled:opacity-50">{working === row.id ? "AI pracuje…" : `Otestovat stranu ${row.ai?.next_page ?? 1}`}</button> : null}
            </div>
          </article>
        ))}
      </div>
      ...
      {review ? (
        <div className="mt-6 rounded-2xl border-2 border-indigo-200 bg-indigo-50/40 p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><h3 className="text-lg font-bold text-slate-900">AI kontrola: {review.retailer} · strana {review.page}/{review.page_count}</h3><p className="text-sm text-slate-600">Bez zápisu do databáze. Bezpečné: {review.approved_count}, karanténa: {review.quarantine_count}.</p></div>
            <div className="flex gap-2"><button type="button" onClick={() => setReview(null)} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-semibold">Zahodit</button><button type="button" disabled={working === review.retailer} onClick={() => void approveReview()} className="rounded-xl bg-emerald-700 px-3 py-2 text-sm font-bold text-white disabled:opacity-50">Schválit stránku a zapsat</button></div>
          </div>
